...
class UploadController:
    def upload_song(mp3_file: UploadFile, image_file: UploadFile, title: str, db: Session, current_user: UserModel):
        folder_name = f"{current_user.id}_{current_user.name}/{mp3_file.filename.split('.')[0]}"
        if title is None:
        ...
        if image_file is not None:
            ...
            rs = s3_upload(Bucket=Bucket, 
                S3Client=S3Connect,
                TargetFilePath=f"{folder_name}/{image_file.filename}",
    ...
    def get_uploaded_song(user_id: int, db: Session):
        upload_song = db.query(SongModel).filter(SongModel.user_id == user_id).all()
